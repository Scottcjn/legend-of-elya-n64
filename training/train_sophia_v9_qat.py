...
# Reuse the dataset + helpers verbatim from v8 so the corpus is bit-identical.
import train_sophia_v8 as v8
...
def build_v7_corpus(seed: int, shard: str = None):
    ...
    v7 has no held-out split, so the 'val' corpus here is an independent shuffle
    of the same lines: it measures memorization, not generalization. Reported as
    such."""
    import ast
    src = open(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "train_sophia_v7.py")).read()
    tree = ast.parse(src)
    lists = {}
    ...
    def build(sd):
        ...
        out = []
        for name, reps in (("IDENTITY_PAIRS", 2800), ("QA_PAIRS", 800), ("CORPUS_LINES", 400)):
            for _ in range(reps):
                ls = list(lists[name])
                rng.shuffle(ls)
                out.extend(ls)
    ...
    return build(seed), build(seed + 7777)


def build_data(cfg_seed: int, train_repeats: int, eval_repeats: int, ctx: int):
    dataset = json.loads(v8.PERSONA_DATASET_JSON)
    v8.validate_dataset(dataset)
    persona_info, order = v8.persona_maps(dataset)
    train_samples, _tbp, eval_by_persona = v8.build_formatted_sets(dataset, persona_info, order)
    train_corpus = v8.build_corpus(train_samples, train_repeats, cfg_seed)
    eval_samples = [s for pid in order for s in eval_by_persona[pid]]
    eval_corpus = v8.build_corpus(eval_samples, eval_repeats, cfg_seed + 1)
    return dataset, persona_info, order, train_corpus, eval_corpus
...
def main() -> None:
    ...
    for step in range(1, args.steps + 1):
        ...
        if step == 1 or step % args.eval_interval == 0 or step == args.steps:
            ...
            hist.append({"step": step, "train": float(loss.item()), "val": float(vl), "fit": float(fit)})
    ...
    meta.update({"tag": tag, "quant": args.quant, "tau": args.tau,
                 ...
                 "seed": args.seed, "corpus": args.corpus, "lr": args.lr, "batch_size": args.batch_size,
                 "weight_decay": args.weight_decay,
