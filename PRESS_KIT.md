...
**A real 819K-parameter GPT transformer runs live inference on a 93.75 MHz Nintendo 64 from 1996 — no lookup tables, no precomputed responses, real attention and softmax on the MIPS R4300i.**

## 50-word blurb

Legend of Elya is an N64 homebrew ROM whose NPC is powered by an actual nano-GPT transformer — 819K parameters, 4 layers, byte-level vocab — computing live on the 93.75 MHz MIPS R4300i. Every response is generated, seeded by hardware oscillator jitter. Built with libdragon. ROM and source are public.
...
| Model | nano-GPT, **819,200 parameters**, 4 layers, 4 heads, dim 128 |
| Vocabulary | 256 (byte-level ASCII) |
| Weights | 458 KB Q8 (int8 + float16 block scales) on cartridge ROM |
| Inference | Float32 on the R4300i FPU; on-the-fly dequantization |
| Speed | ~60 tok/s (ares emulator) · 1–3 tok/s real hardware, scalar · RSP vector build included |
...
- The headline 60 tok/s figure is **emulator** speed; stock real hardware runs 1–3 tok/s on the scalar CPU. The RSP microcode build is faster; real-hardware video capture of the EverDrive 64 run is still pending.
- 819K parameters is a *nano* model: coherent short English responses to 32 themed prompts, not general conversation.
...
| 0:35–0:60 | **The money shot**: tokens appearing character-by-character, live tok/s counter on screen. Let it breathe — no cuts for 15+ seconds. | Caption: "819,200-parameter GPT transformer. Live inference. Real attention. Real softmax." |
| 0:60–0:75 | Quick-cut montage: code (Taylor exp, fast inverse sqrt), the 458 KB weight file, RSP microcode scrolling. | Caption: "No lookup tables. No network. The cartridge IS the model." |
